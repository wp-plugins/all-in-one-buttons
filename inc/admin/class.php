 ...
 */
  ...
  class aiobtnAdmin {
    ...
    /**
      ...
      public function DisplayPage() {
        ...
        wp_nonce_field( basename(__FILE__), 'AIOBTN_Admin_Page_Class_nonce' );

        if ($this->saved_flag)
          echo '<div class="alert aiobtn-success"><p><strong>'.__('Settings saved.', 'aiobtn').'</strong></p></div>';
      ...
      echo '</div><div style="clear:both"></div><div class="footer_wrap">
          <div style="float:right;margin:0 0 0 0">
            <input type="submit" style="margin-left: 20px;" name="Submit" class="button" value="'.esc_attr(__('Save Changes', 'aiobtn')).'" />
            <br><br>
          </div>
          <br style="clear:both"><br>
        </div>';
  ...
  public function insert_images() {
    ...
    foreach( $_POST['attachments'] as $attachment_id => $attachment ) {
      ...
      //$li   .= "<a title='" . __( 'Delete this image', 'aiobtn' ) . "' class='at-delete-file' href='#' rel='{$nonce}|{$post_id}|{$id}|{$attachment_id}'>" . __( 'Delete', 'aiobtn' ) . "</a>";
      $li   .= "<a title='" . __( 'Delete this image', 'aiobtn' ) . "' class='at-delete-file' href='#' rel='{$nonce}|{$post_id}|{$id}|{$attachment_id}'><img src='" . $this->SelfPath. "/images/delete-16.png' alt='" . __( 'Delete', 'aiobtn' ) . "' /></a>";
  ...
  */
  public function wp_ajax_delete_image() {
    ...
    if ( $ok ){
      ...
      echo json_encode(array('message' => __( 'Cannot delete file. Something\'s wrong.', 'aiobtn')));
      die();
    }
  }
  ...
  public function show_field_repeater( $field, $meta ) {
    ...
    $meta = isset($this->_saved[$field['id']])? $this->_saved[$field['id']]: '';
      ...
      if (count($meta) > 0 && is_array($meta) ){
        ...
        echo '" alt="'.__('Remove', 'aiobtn').'" title="'.__('Remove', 'aiobtn').'" id="remove-'.$field['id'].'"></div>';
        $c = $c + 1;
    ...
    echo '" alt="'.__('Add', 'aiobtn').'" title="'.__('Add', 'aiobtn').'" id="add-'.$field['id'].'"><br/></div>';
...
.at-repater-block{background-color: #FFFFFF;border: 1px solid;margin: 2px;}
  ...
  public function show_field_file( $field, $meta ) {
    ...
    $this->show_field_begin( $field, $meta );
      ...
      if ( ! empty( $meta ) ) {
        $nonce = wp_create_nonce( 'at_ajax_delete' );
        echo '<div style="margin-bottom: 10px"><strong>' . __('Uploaded files', 'aiobtn') . '</strong></div>';
        echo '<ol class="at-upload">';
        foreach ( $meta as $att ) {
          // if (wp_attachment_is_image($att)) continue; // what's image uploader for?
          echo "<li>" . wp_get_attachment_link( $att, '' , false, false, ' ' ) . " (<a class='at-delete-file' href='#' rel='{$nonce}|{$post->ID}|{$field['id']}|{$att}'>" . __( 'Delete', 'aiobtn' ) . "</a>)</li>";
        }
        echo '</ol>';
      }
      ...
      echo "<div class='at-file-upload-label'>";
        echo "<strong>" . __( 'Upload new files', 'aiobtn' ) . "</strong>";
      echo "</div>";
      echo "<div class='new-files'>";
        echo "<div class='file-input'>";
          echo "<input type='file' name='{$field['id']}[]' />";
        echo "</div><!-- End .file-input -->";
        echo "<a class='at-add-file button' href='#'>" . __( 'Add more files', 'aiobtn' ) . "</a>";
  ...
  public function show_field_typo( $field, $meta ) {
    ...
    $html .= "<input type='button' class='at-color-select button' rel='".$field['id']."[color]' value='" . __( 'Select a color', 'aiobtn' ) . "'/>";
  ...
  public function show_field_color( $field, $meta ) {
    ...
    $this->show_field_begin( $field, $meta );
      ...
      echo "<input type='button' class='at-color-select button' rel='{$field['id']}' value='" . __( 'Select a color', 'aiobtn' ) . "'/>";
  ...
  public function save($repeater = false) {
    ...
    //check nonce
    if ( ! check_admin_referer( basename( __FILE__ ), 'AIOBTN_Admin_Page_Class_nonce') )
      return;
    ...
    foreach ( $this->_fields as $field ) {
      if(!in_array($field['type'],$skip)){
        ...
        // Validate meta value
        if ( class_exists( 'AIOBTN_Admin_Page_Class_Validate' ) && method_exists( 'AIOBTN_Admin_Page_Class_Validate', $field['validate_func'] ) ) {
          $new = call_user_func( array( 'AIOBTN_Admin_Page_Class_Validate', $field['validate_func'] ), $new );
        }
...
require_once(aiobtn_path .'inc/admin/options.php');
